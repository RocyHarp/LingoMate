...
st.markdown("""
    ...
    .top-bar-container {
        ...
        z-index: 9999; display: flex; align-items: center; padding-left: 20px;
    }

    /* ФІКСАЦІЯ МЕНЮ ЗЛІВА (Щоб воно не падало вниз) */
    ...
    /* КНОПКА МЕНЮ */
    div.stButton > button.menu-btn {
        background-color: #3b82f6; color: white; border: none; border-radius: 12px;
        height: 50px; width: 50px; font-size: 24px; transition: all 0.2s ease;
        box-shadow: 0 4px 6px rgba(59, 130, 246, 0.4);
        margin-top: 0px; 
    }
    div.stButton > button.menu-btn:hover {
    ...
    /* БОКОВЕ МЕНЮ (БЕЗ РАМОК І КВАДРАТІВ) */
    .custom-sidebar {
    ...
    .menu-stat-num { font-size: 20px; font-weight: bold; color: #f8fafc; }
    .menu-stat-label { font-size: 11px; color: #94a3b8; text-transform: uppercase; margin-top: 2px;}

    /* РІВЕНЬ (Level Bar) */
    .level-container {
        background-color: #0f172a; padding: 15px; border-radius: 12px; border: 1px solid #334155; margin-bottom: 20px;
    }
    .level-header { display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 13px; color: #cbd5e1; }
    .progress-track { width: 100%; height: 8px; background-color: #334155; border-radius: 4px; overflow: hidden; }
    .progress-fill { height: 100%; background: linear-gradient(90deg, #3b82f6, #8b5cf6); border-radius: 4px; width: 65%; }

    /* КАРТКИ */
    .friend-card {
        background: #0f172a; padding: 12px; border-radius: 10px;
        border: 1px solid #334155; margin-bottom: 8px; display: flex; align-items: center; gap: 10px;
    }
    .vocab-row { background-color: #1e293b; padding: 8px 12px; border-radius: 8px; margin-bottom: 8px; border-left: 3px solid #3b82f6; display: flex; align-items: center; justify-content: space-between; }
...
if col_menu:
    with col_menu:
        ...
        st.code(f"ID: {st.session_state.user_id}", language="text")
        
        # 2. Рівень
        st.markdown("""
        <div class="level-container">
            <div class="level-header">
                <span><b>Рівень 5</b></span>
        ...
        <div style="display: flex; gap: 10px; margin-bottom: 20px;">
            ...
            <div class="menu-stat-box" style="flex: 1;">
                <div class="menu-stat-num">{stats['translations']}</div>
        ...
        # 4. Друзі
        st.markdown('**👥 МОЇ ДРУЗІ**')
        new_f = st.text_input("ID друга", label_visibility="collapsed", placeholder="Введи ID...")
        if st.button("➕ Додати", use_container_width=True, key="add_friend_btn"):
        ...
        if friends:
            for f in friends:
                ...
                <div class="friend-card">
                    <div style="background:#3b82f6; width:30px; height:30px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:16px;">👤</div>
                    <div style="flex:1;">
                        <div style="font-weight:bold; font-size:14px;">{f['username']}</div>
                        <div style="font-size:11px; color:#94a3b8;">{f['total_words']} слів</div>
                    </div>
                </div>""", unsafe_allow_html=True)
        else: st.caption("Список друзів порожній")

        st.divider()
        
        # 5. Налаштування
        with st.expander("⚙️ Акаунт"):
            np = st.text_input("Новий пароль", type="password", key="change_pass_in")
            if st.button("Зберегти пароль"):
                if db.change_password(st.session_state.user_id, np): st.success("Ок")
            st.markdown("---")
            if st.button("🗑️ Видалити профіль", type="primary", key="del_acc_btn"):
...
with col_main:
    ...
    with t_trans:
        ...
        with c1:
            ...
            text_input = st.text_area("Введення", value=st.session_state.input_text, height=text_height, placeholder="Введіть текст...", label_visibility="collapsed")
    ...
    with t_train:
        ...
        with c_source:
            train_source = st.radio("Джерело:", ["🎲 Випадкове", "🧠 Розумне"])
        with c_mode:
            mode = st.radio("Режим:", ["Картки", "Письмо EN-UA", "Письмо UA-EN"])
        with c_count:
            count_opt = st.radio("Кількість:", [10, 30, 50, "∞ Всі"])
        ...
        with c_go:
            ...
            if st.button("🔄 Почати", type="primary", use_container_width=True):
                ...
                if not st.session_state.train_session and all_words_flat:
        ...
        if 'train_session' in st.session_state and st.session_state.train_session:
            ...
            if idx >= total_q:
                 ...
                 with st.container(border=True):
                     ...
                     elif percent >= 80: msg = "😎 Чудовий результат!"
                ...
                # ВИПРАВЛЕНО: Цифри тепер над повзунком
                ...
                with st.container(border=True):
                    ...
                    if is_synonym and curr['nuance'] and "UA-EN" not in mode:
