...
# --- CSS ТА JS (ЯДЕРНИЙ ВАРІАНТ) ---
# Ми створюємо свою кнопку і скрипт для керування меню
st.markdown("""
    <style>
        /* 1. Відступи для контенту (щоб не налізав на шапку) */
        .block-container {
            padding-top: 5rem !important;
            padding-bottom: 2rem !important;
        }

        /* 2. Ховаємо ВСЕ системне, що може заважати */
        #MainMenu {visibility: hidden;}
        footer {visibility: hidden;}
        header {visibility: hidden;} /* Ховаємо рідний хедер */
        [data-testid="stToolbar"] {display: none !important;}
        [data-testid="stDecoration"] {display: none;}
        
        /* Ховаємо рідну кнопку меню (ми зробимо свою) */
        [data-testid="stSidebarCollapsedControl"] {
            display: none !important;
        }

        /* 3. НАША КАСТОМНА КНОПКА МЕНЮ (CSS) */
        .custom-menu-btn {
            position: fixed;
            top: 15px;
            left: 20px;
    ...
    <script>
        function toggleSidebar() {
            // Знаходимо рідну кнопку меню (навіть якщо вона прихована)
            const sidebarBtn = window.parent.document.querySelector('[data-testid="stSidebarCollapsedControl"]');
            if (sidebarBtn) {
                sidebarBtn.click();
            } else {
                // Запасний варіант для мобільних/інших версій
...
if "user_id" not in st.session_state:
    # На екрані входу показуємо заголовок по центру, без кнопки меню зліва
    _, cent, _ = st.columns([1, 1.5, 1])
    with cent:
        st.markdown("<div style='display:flex; justify-content:center; align-items:center; gap:10px; margin-bottom:20px;'><div style='font-size:40px; background:#1e293b; padding:10px; border-radius:12px; border:1px solid #334155;'>⚡</div><h1 style='margin:0;'>LingoMate</h1></div>", unsafe_allow_html=True)
        ...
        with tab_reg:
            ...
            if st.button("Створити акаунт"):
                if not ru or not re or not rp or not rp2:
                    ...
                    st.error("Логін або пошта вже зайняті.")
    st.stop()


# --- ГОЛОВНИЙ ЕКРАН (Тільки після входу) ---

# 1. Вставляємо нашу кнопку меню і заголовок через HTML
# onclick="toggleSidebar()" викликає наш JS скрипт
st.markdown("""
    <button class="custom-menu-btn" onclick="toggleSidebar()">⚡</button>
    <div class="custom-title">LingoMate</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with st.expander("🤝 Мої Друзі", expanded=True):
        ...
        new_friend_id = st.text_input("Додати друга (ID)", placeholder="Введи ID...", label_visibility="collapsed")
        if st.button("Додати"):
        ...
        if friends:
            ...
            st.info("Список друзів порожній.")
...
with t_trans:
    ...
    with c1:
        ...
        text_input = st.text_area("Введення", value=st.session_state.input_text, height=text_height, placeholder="Type something...", label_visibility="collapsed")
