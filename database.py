...
def update_user_avatar(user_id, avatar):
    ...
    try:
        ...
        try:
            ...
            conn.rollback() # У Postgres важливо робити відкат після помилки в транзакції
...
def change_username(user_id, new_username):
    ...
    try:
        ...
        return False # Такий нікнейм вже існує
    except:
        if conn: conn.rollback()
        return False
    finally:
...
def get_user_bonus_xp(user_id):
    ...
    try:
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("ALTER TABLE users ADD COLUMN bonus_xp INTEGER DEFAULT 0")
            conn.commit()
        except Exception:
            conn.rollback()
...
def add_user_bonus_xp(user_id, amount):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE users SET bonus_xp = COALESCE(bonus_xp, 0) + %s WHERE id = %s", (amount, user_id))
        conn.commit()
    except:
        if conn: conn.rollback()
    finally:
