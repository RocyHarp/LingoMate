...
def get_db_connection(): return get_pool().getconn()
def release_db_connection(conn): get_pool().putconn(conn)
...
def init_db():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute('''CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, username TEXT UNIQUE, email TEXT UNIQUE, password TEXT, is_premium INTEGER DEFAULT 0, avatar TEXT DEFAULT '🦊', bonus_xp INTEGER DEFAULT 0)''')
        cursor.execute('''CREATE TABLE IF NOT EXISTS words (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id) ON DELETE CASCADE, word TEXT, meaning TEXT, word_type TEXT, correct_count INTEGER DEFAULT 0, wrong_count INTEGER DEFAULT 0, parent_id INTEGER DEFAULT NULL, nuance TEXT DEFAULT NULL, dialect TEXT DEFAULT 'Global')''')
        ...
        # Перевірка для нових колонок (якщо база вже була створена)
        try: cursor.execute("ALTER TABLE words ADD COLUMN IF NOT EXISTS dialect TEXT DEFAULT 'Global'")
        except: pass
...
def change_password(user_id, new_password):
    ...
    try:
        ...
        cursor.execute('UPDATE users SET password = %s WHERE id = %s', (hashed_pw, user_id)); conn.commit(); return True
    except: return False
    finally:
        if conn: release_db_connection(conn)
...
def delete_account(user_id):
    ...
    try:
        conn = get_db_connection(); cursor = conn.cursor()
        cursor.execute('DELETE FROM users WHERE id = %s', (user_id,)); conn.commit(); return True
    except: return False
    finally:
        if conn: release_db_connection(conn)
...
def get_username(user_id):
    ...
    try:
        ...
        res = cursor.fetchone(); return res[0] if res else "User"
    except: return "User"
    finally:
        if conn: release_db_connection(conn)
...
def get_word_count(user_id):
    ...
    try:
        ...
        res = cursor.fetchone(); return res[0] if res else 0
    except: return 0
    finally:
        if conn: release_db_connection(conn)
...
def get_words_for_smart_training(user_id, limit=10):
    ...
    try:
        conn = get_db_connection(); cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute('''SELECT * FROM words WHERE user_id = %s ORDER BY (wrong_count + 1) * RANDOM() DESC LIMIT %s''', (user_id, limit))
        return [dict(row) for row in cursor.fetchall()]
    except: return []
    finally:
        if conn: release_db_connection(conn)
...
def save_word(user_id, word, meaning, word_type, dialect='Global'):
    conn = None
    try:
        conn = get_db_connection(); cursor = conn.cursor()
        cursor.execute('SELECT id FROM words WHERE user_id = %s AND word = %s', (user_id, word.strip()))
        if cursor.fetchone(): return False
        cursor.execute('INSERT INTO words (user_id, word, meaning, word_type, correct_count, wrong_count, dialect) VALUES (%s, %s, %s, %s, 0, 0, %s)', (user_id, word.strip(), meaning.strip(), word_type.strip(), dialect))
...
def save_synonym(user_id, parent_id, word, meaning, nuance):
    ...
    try:
        conn = get_db_connection(); cursor = conn.cursor()
        cursor.execute('''INSERT INTO words (user_id, word, meaning, word_type, correct_count, wrong_count, parent_id, nuance, dialect) VALUES (%s, %s, %s, 'synonym', 0, 0, %s, %s, 'Global')''', (user_id, word.strip(), meaning.strip(), parent_id, nuance.strip()))
        conn.commit(); clear_db_cache(); return True
    except: return False
    finally: 
        if conn: release_db_connection(conn)
...
def get_user_avatar(user_id):
    ...
    try:
        ...
        res = cursor.fetchone(); return res[0] if res and res[0] else '🦊'
    except: return '🦊'
    finally:
        if conn: release_db_connection(conn)
...
def change_username(user_id, new_username):
    ...
    try:
        conn = get_db_connection(); cursor = conn.cursor()
        cursor.execute("UPDATE users SET username = %s WHERE id = %s", (new_username, user_id)); conn.commit(); clear_db_cache(); return True
    except:
        if conn: conn.rollback()
        return False
    finally:
...
def get_user_bonus_xp(user_id):
    conn = None
    try:
        conn = get_db_connection(); cursor = conn.cursor()
        cursor.execute("SELECT bonus_xp FROM users WHERE id = %s", (user_id,))
        res = cursor.fetchone(); return res[0] if res and res[0] else 0
    except: return 0
    finally:
        if conn: release_db_connection(conn)

def add_user_bonus_xp(user_id, amount):
    conn = None
    try:
        conn = get_db_connection(); cursor = conn.cursor()
        cursor.execute("UPDATE users SET bonus_xp = COALESCE(bonus_xp, 0) + %s WHERE id = %s", (amount, user_id)); conn.commit(); clear_db_cache()
    except:
        if conn: conn.rollback()
    finally:
        if conn: release_db_connection(conn)
def get_word_by_text(user_id, word_text):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT id FROM words WHERE user_id = %s AND word = %s', (user_id, word_text.strip()))
        return cursor.fetchone()
